...
ggmap(get_map(location = c(75, 20, 110, 50), map_type = "terrain", source = "stamen"))+
  annotate("rect", xmin = 75, xmax = 110, ymin = 20, ymax = 50, fill = "white", alpha = 0.4)+
  geom_point(data = data8, mapping = aes(x = x, y = y), colour = "deepskyblue4", size = 3.5) +
  geom_point(data = data8, mapping = aes(x = x, y = y), colour = "deepskyblue", size = 2) +
...
ggmap(get_map(location = c(75, 20, 110, 50), map_type = "terrain", source = "stamen"))+
  annotate("rect", xmin = 75, xmax = 110, ymin = 20, ymax = 50, fill = "white", alpha = 0.4)+
  geom_point(data = data9, mapping = aes(x = x, y = y), colour = "dodgerblue4", size = 3.5) +
  geom_point(data = data9, mapping = aes(x = x, y = y), colour = "dodgerblue", size = 2) +
  labs(title = "Locations of state 4")
